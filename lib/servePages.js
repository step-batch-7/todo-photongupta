...
const todoHistory = require(DATA_BASE);
...
const createNewTodo = function(req) {
  ...
  todo.id = generateId();
  todo.todoItems = new Array(todo.todoItem)
    .flat()
    .map(item => new Item(item, generateId()));
  return new Todo(todo);
};

const toggleIsDoneStatus = function(ids) {
  ids = ids[0].split(',').map(id => +id);
  console.log(ids);
  const modifiedHistory = todoHistory.map(todo => {
...
const showTodoList = function(req, res) {
  const titlesInHtml = todoHistory
